...
func (b *bucket[T]) SetIfNotExists(key string, value T, duration time.Duration, track bool) *Item[T] {
	b.RLock()
	item := b.lookup[key]
	b.RUnlock()
	if item != nil {
...
func (b *bucket[T]) SetIfNotExistsWithFunc(key string, f func() T, duration time.Duration, track bool) (*Item[T], bool) {
	b.RLock()
	item := b.lookup[key]
	b.RUnlock()
	if item != nil {
