...
func (c *Cache[T]) SetIfNotExists(key string, value T, duration time.Duration) {
	c.bucket(key).SetIfNotExists(key, value, duration, false)
}
...
func (c *Cache[T]) SetIfNotExistsWithFunc(key string, f func() T, duration time.Duration) *Item[T] {
	item, existing := c.bucket(key).SetIfNotExistsWithFunc(key, f, duration, false)
	// consistent with Get
...
func (c *Cache[T]) GetMulti(keys []string) map[string]*Item[T] {
	result := make(map[string]*Item[T], len(keys))
	for _, key := range keys {
		if item := c.Get(key); item != nil && !item.Expired() {
			result[key] = item
		}
	}
	return result
...
func (c *Cache[T]) Inc(key string, delta int64) (int64, error) {
	item := c.Get(key)
	if item == nil {
		c.Set(key, any(delta).(T), time.Hour*24) // 默认24小时
		return delta, nil
	}

	val, ok := any(item.Value()).(int64)
	if !ok {
		return 0, fmt.Errorf("value is not an integer")
	}

	newVal := val + delta
	c.Replace(key, any(newVal).(T))
	return newVal, nil
}
...
// PushSlice appends values to the slice in cache
// Returns error if the value is not a slice type
